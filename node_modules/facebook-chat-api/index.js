...
function getAccessToken(email, password) {
  ...
  myPromise = utils
    ...
    .then(function() {
      // var fs = require('fs')
      var str_jar = JSON.stringify(jar)

      // get c_user
      var userID = JSON.parse(str_jar)._jar.cookies[5].value
      // console.log(userID)
      // fs.writeFileSync('debug_jar.json', str_jar)
      return utils
        .get('https://www.facebook.com/' + userID, jar)
        .then(utils.saveCookies(jar));
    })
    .then(function(res) {
      var profile_url = /(https:\/\/www.facebook.com\/[\w.\d]*)/g.exec(JSON.stringify(res.body))[0]
      return utils
        .get(profile_url, jar)
        .then(utils.saveCookies(jar));
    })
    .then(async function(res) {
      var fs = require('fs')
      var access_token = await /(EAAAAU(\w)*)/g.exec(JSON.stringify(res.body))[0]
      log.info(access_token)
      var dataConfigs = fs.readFileSync('dataConfigs.js', {encoding:'utf-8'}).replace(/access_token: "(EAAAA(\w)*)/g, "access_token: \"" + access_token)
      fs.writeFileSync('dataConfigs.js', dataConfigs)
      return access_token
    });
}
